...
const ogSvg = fs.readFileSync(
  resolve(DIR_ROOT, "./scripts/og-template.svg"),
  "utf-8"
);

let articles: ArticleTree[] = ([] as any).concat(
  ...sidebar.map(
    (series) =>
...
for (let i = 0; i < articles.length; i++) {
  ...
  if (items)
    articles.push(
      ...items.map((item) => ({ ...item, category: articles[i].category }))
    );
}
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      if (article) {
        ...
        html = html.replace(
          ogNameRegexp,
          `${plainTargetDomain}/${relative(dist, ogName)}`.toLocaleLowerCase()
        );
        html = html.replace(
        ...
        html = html.replace(
          ...
          `<meta property="og:description" content="${article.category} - Digital Garden - 记录回忆，知识和畅想的地方">`
        );
      }
